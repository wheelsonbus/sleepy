...
{
    struct zz_internal_palette_config internal_palette_config;
    internal_palette_config.path = config->path;
    if (!zz_internal_palette_create(&palette->internal_palette, &internal_palette_config))
    {
        return ZZ_FALSE;
    }
...
{
    zz_internal_palette_destroy(&palette->internal_palette);
}